...
namespace _2DLevelCreator
{
    public class GameInput
    {
        ...
        public GameInput(XnaWindow xnaWindow)
        {
        ...
        public bool CheckPressed(ButtonState current, ButtonState previous)
        {
            if (current == ButtonState.Pressed && previous == ButtonState.Released) return true;
            else return false;
        }
        public bool CheckReleased(ButtonState current, ButtonState previous)
        {
            if (current == ButtonState.Released && previous == ButtonState.Pressed) return true;
            else return false;
        }
        public bool CheckIsDown(ButtonState current)
        {
        ...
        {
            // Scale the coordinates to match the actual size of the control's window. 
            // The Graphics Device seems to be shared across all controls, so scaling is required 
            // to adjust for the difference between the Graphics Device's viewport size and the control's window size.
            /*
            float xScaler = (float)((XnaWindow)(control)).Editor.GraphicsDevice.Viewport.Width / (float)((XnaWindow)(control)).Width;
            float yScaler = (float)((XnaWindow)(control)).Editor.GraphicsDevice.Viewport.Height / (float)((XnaWindow)(control)).Height;


            return new Vector3(mouseStateCurrent.X * xScaler, mouseStateCurrent.Y * yScaler, 0);
            */
        ...
        public void Update()
        {
            ...
            Mouse.WindowHandle = this.xnaWindow.Handle;   //Needed for MouseWheel

            this.mousePositionPrevious = this.mousePositionCurrent;
            this.mousePositionCurrent = this.GetMousePos(this.xnaWindow, Vector3.Zero);


            this.mousePositionPreviousProjected = this.mousePositionCurrentProjected;
            this.mousePositionCurrentProjected = this.GetWorldMouseCoords_XYPlane(this.xnaWindow.camera, this.mousePositionCurrent, 0, this.xnaWindow.Width, xnaWindow.Height); //This needs to be calculated after any camera movement.
            ...
            this.MouseLeftDownOnXnaWindow = (MouseLeftPressed && this.xnaWindow.Focused) ? true : false;
